...
object Json {
  ...
  type Parser = State => Xor[Error, State]
  ...
  private def expr(state: State)(implicit converters: Converters): Xor[Error, State] =
    value(state) orElse arr(state) orElse obj(state)

  private def value[T](state: State)(implicit converters: Converters): Xor[Error, State] = {
...
//      val value = converter(valueString)
  ...
  private def arr(state: State)(implicit converters: Converters): Xor[Error, State] =
    for {
      s1 <- symbol("[", state)
      s2 <- repeat(expr, s1, ",")
      s3 <- symbol("]", s2)
  ...
  private def obj(state: State)(implicit converters: Converters): Xor[Error, State] =
    for {
      s1 <- symbol("{", state)
      s2 <- repeat(mapEntry, s1, ",")
      s3 <- symbol("}", s2)
    } yield mapEntriesToMap(s3)

  def mapEntriesToMap(state: State): State = {
  ...
  private def repeat(parser: Parser, state: State, separator: String)(implicit converters: Converters): Xor[Error, State] = {
    val first: Xor[Error, State] = parser(state)
    if (first.isLeft)
  ...
  private def mapEntry(state: State)(implicit converters: Converters): Xor[Error, State] =
    for {
      s1 <- key(state)
      s2 <- symbol(":", s1)
      s3 <- expr(s2)
  ...
  private def key(state: State): Xor[Error, State] = {
    val option = """^"(.+?)"""".r.findFirstMatchIn(state.jsonLeft)
    if (option.isDefined) {
  ...
  def symbol(symbol: String, state: State): Xor[Error, State] =
  ...
  case class State(jsonLeft: JsonString, data: Any) {
    def advance(chars: Int) =
      State(jsonLeft.substring(chars), data)

    def advance(chars: Int, newData: Any) =
