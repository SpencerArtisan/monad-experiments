...
object Json {
  ...
  private def text(state: State[Any])(implicit converters: Converters): Xor[Error, State[String]] = {
...
//      val value = converter(valueString)
  ...
  private def repeat[B <: Any](parser: Parser[Any, B], state: State[Any], separator: String)(implicit converters: Converters): Xor[Error, State[List[B]]] = {
    val first: Xor[Error, State[B]] = parser(state)
    if (first.isLeft)
